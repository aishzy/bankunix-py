...
class AccountRepository:
    ...
    def create_account(self, account: Account) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO accounts (account_id, user_id, account_type, account_number, balance, currency, created_at, is_active, interest_rate)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (account.account_id, account.user_id, account.account_type, account.account_number, account.currency, account.created_at, account.is_active, account.interest_rate))
            conn.commit()
            return True
    ...
    def get_account_by_user(self, user_id: str) -> List[Account]:
    ...
    def _map_to_account(row: sqlite3.Row) -> Account:
        ...
        return Account(
            ...
            created_at = datetime.fromisoformat(row['createad_at']),
