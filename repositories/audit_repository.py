...
class TransferRepository:
    ...
    def create_transfer(self, transfer: Transfer) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO values (transfer_id, from_account_id, to_account_id, 
                                    amount, description, created_at, completed_at, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            '''), (transfer.transfer_id, transfer.from_account_id, transfer.to_account_id, transfer.amount, 
                   transfer.description, transfer.created_at, transfer.completed_at, transfer.status.value)
            conn.commit()
            return True
        except sqlite3.Error:
            return False

    def get_transfers_by_id(self, transfer_id: str) -> Optional[Transfer]:
    ...
    def get_pending_transfers(self) -> List[Transfer]:
        ...
        conn = self.db_managet.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT * FROM transfers
            WHERE status = ?
    ...
    def _map_to_transfer(row: sqlite3.Row) -> Transfer:
        """Map database row to transfer object"""
        return Transfer(
...
class BillRepository:
    ...
    def create_bill(self, bill: Bill) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO bills (bill_id, account_id, biller_name, amount, 
                                   due_date, status, created_at, paid_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (bill.bill_id, bill.account_id, bill.biller_name, bill.amount, bill.due_date, bill.status, bill.created_at, bill.paid_at))
            conn.commit()
            return True
        except sqlite3.Error:
            return False
    ...
    def get_bills_by_accounts(self, account_id: str) -> List[Bill]:
        """Get all bills for an account"""
        conn = self.db_manager.get_account()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT * FROM bills
            WHERE account_id = ?
